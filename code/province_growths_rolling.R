...
dates <- c(seq(from = first_date, to = last_date, by = 60*60*24), last_date)
...
for(i in 1:(length(dates)-7)){
  use.i <- which(dat.combine$DATE >= dates[i] & dat.combine$DATE < dates[i+6] & dat.combine$PROV != "Hubei")
...
times <- rep(dates[-c(1:7)], times = unlist(lapply(doubling_prov, function(x) length(x))))
fill1 <- rep("1 - before", times =  15*length(unique(prov)))
fill2 <- rep("2 - after", times =  23*length(unique(prov)))
fill <- c(fill1, fill2)
